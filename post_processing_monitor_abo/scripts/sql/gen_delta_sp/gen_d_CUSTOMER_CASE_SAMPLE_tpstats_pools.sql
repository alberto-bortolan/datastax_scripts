DROP PROCEDURE IF EXISTS gen_d_<CUSTOMER_NAME>_<CASE_NUMBER>_<SAMPLE_SET>_tpstats_pools;
DELIMITER //
 
CREATE PROCEDURE gen_d_<CUSTOMER_NAME>_<CASE_NUMBER>_<SAMPLE_SET>_tpstats_pools()
BEGIN
  ...
  DECLARE curs1 CURSOR FOR SELECT  runid, ip, pool, ts, completed FROM for_grafana.<CUSTOMER_NAME>_<CASE_NUMBER>_<SAMPLE_SET>_tpstats_pools ORDER BY runid,ip,pool,ts;

  DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;  
  
  SET @updsql = 'UPDATE for_grafana.<CUSTOMER_NAME>_<CASE_NUMBER>_<SAMPLE_SET>_tpstats_pools SET d_completed = ? WHERE runid = ? AND ip = ? AND pool = ? AND ts = ?'; 
  ...
  OPEN curs1;
  read_loop: LOOP
    FETCH curs1 INTO c_runid, c_ip, c_pool, c_ts, c_completed;
    IF done THEN
      LEAVE read_loop;
    END IF;
	SET nloop = nloop+1;
    ...
    EXECUTE updstm USING vd_completed,c_runid,c_ip,c_pool, c_ts;
	-- set 1st row like the 2nd to avoid the starup ramp in the graphs
	IF nloop = 2 THEN
	   EXECUTE updstm USING vd_completed, first_ip, first_runid, first_pool, first_ts;
	END IF;
	SET p_ts = c_ts;
